...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def shopping_cart(self, request, pk=None):
        ...
        if request.method == 'POST':
            recipe_in_cart, created = ShoppingCart.objects.get_or_create(user=request.user,
                                                                         recipe=recipe)
    ...
    def favorite(self, request, pk=None):
        recipe = self.get_object()
        if request.method == 'POST':
            recipe_in_favorite, created = Favorite.objects.get_or_create(user=request.user,
                                                                         recipe=recipe)
